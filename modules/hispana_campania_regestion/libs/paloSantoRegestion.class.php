...
class paloSantoRegestion{
	...
	// $DATA['id'] tiene el id de la campaña padre (campania_origen)
        ...
        $query= "select * from (SELECT c.* FROM campania_cliente AS c where (c.id_campania_consolidada=$DATA[id] or c.id_campania=$DATA[id]) AND c.status in ('Gestion','Display') order by c.id desc) x group by x.ci;";
	...
	//$this->_log->output("paloSantoRegestion->saveNewRegestion: Select a campania_cliente \n" . $query);
        ...
        //Se agrega para verificar si es tipo "No contactado"
        $no_contactado=false;
        $query = "select clase from calltype where id in (".$DATA['values_calltypes'].") limit 1";
	$resultCalltype = $this->_DB->getFirstRowQuery($query, true);
        if($resultCalltype["clase"]=="No contactado"){
            $no_contactado=true;
        }
	...
	foreach($resultCampaniaCliente as $campania_cliente){
            $where="";
            if($no_contactado){
                $where="and not exists(select * from gestion_campania gc,calltype c where gc.calltype=c.id and gc.id_campania_cliente=$campania_cliente[id] and c.clase<>'No contactado')";
            }
	    $query = "SELECT * from gestion_campania
		      WHERE id_campania_cliente=$campania_cliente[id]
		      AND calltype in ($DATA[values_calltypes]) $where";
