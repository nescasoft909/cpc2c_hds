...
class paloSantoReportedeCalltypes{
	...
	/*$query   = "SELECT distinct ci
        ...
        $query = "SELECT  
		      ...
		      cliente AS e,
                      ...
                      campania_base g
		      WHERE 
		      (a.id_campania_consolidada = $id_campania or a.id_campania=$id_campania) AND 
		      a.id_gestion_mejor_calltype = b.id AND 
		      b.calltype = c.id AND
		      $id_campania = d.id AND
		      a.ci=e.ci AND
                      f.id=g.id_base AND
                      (g.id_campania=a.id_campania_consolidada or g.id_campania=a.id_campania)
                      $where";
	...
	foreach($result_ci as $arrCi){
	    $query = "SELECT  
		      ...
		      b.id as 'id_gestion_mejor_calltype',
		      b.calltype, 
